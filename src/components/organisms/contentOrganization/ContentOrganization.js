...
export const ContentOrganization = ({ data }) => {
  const { data: dataOrganization, isLoading, isFetching, error } = data;
  ...
  return (
    <Container>
      ...
      <div className="mb-10" data-aos="fade-left">
        <div className="mb-3">
          <SearchBar
            defaultValue={state.formGlobal.searchOrganization}
            onChange={(e) => setTxtSearch(e.target.value)}
            onClick={() =>
              dispatch(setFormGlobal("searchOrganization", txtSearch))
            }
          />
        </div>
        <div className="flex items-center justify-between" data-aos="zoom-in">
          <div className="flex items-center gap-4">
            <h1 className="text-[#22D4EC] font-bold text-lg">
              Tersaji 1 - 6 dari {dataOrganization.TotResult} Hasil :
            </h1>
            {isFetching && <Spinner />}
          </div>
          <select
      ...
      <div className="grid md:grid-cols-3 gap-10 grid-cols-1">
        {dataOrganization?.Datasets?.map((item, index) => {
          return (
