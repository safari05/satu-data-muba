...
import { ProfileCategory } from "../molecules";
import { useContext, useEffect, useState } from "react";
import { GlobalContext, setFormGlobal } from "@/context";
import { useDatasetCategoryDatasetQuery } from "@/hooks";

const DetailCategoryScreen = ({ initialData, slug }) => {
  const [initData, setInitData] = useState(initialData);
  const [pageIndex, setPageIndex] = useState(0);
  const [txtSearch, setTxtSearch] = useState("");
  const { state, dispatch } = useContext(GlobalContext);
  var rq = useDatasetCategoryDatasetQuery(
    slug,
